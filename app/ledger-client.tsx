...
import { ArrowUpRight, BellRing, CalendarClock, ChevronRight, Download, FileCheck2, FileSpreadsheet, Search, Upload, X } from "lucide-react";
...
const statusMeta: Record<Status, { label: string; className: string; dot: string }> = {
  ...
  upcoming: { label: "即将实施", className: "border-amber-200 bg-amber-50 text-amber-700", dot: "bg-amber-500" },
  replaced: { label: "已被替代", className: "border-slate-200 bg-slate-100 text-slate-600", dot: "bg-slate-400" },
...
function getComparisonInputMeta(row: SpreadsheetRow) {
  return {
    ...
    originalPublished: getRowValue(row, ["发布日期", "发布日", "publication"]),
    originalEffective: getRowValue(row, ["实施日期", "生效日期", "effective"]),
    originalSource: getRowValue(row, ["来源", "来源机构", "发布机构", "source"]),
...
function addComparisonProvenance(rows: SpreadsheetRow[], sourceRegulations: Regulation[] = regulations): ComparisonRow[] {
  return compareRows(rows, sourceRegulations).map((item, index) => {
    ...
    return {
      ...item,
      ...inputMeta,
      referenceStatus: reference ? statusMeta[reference.status].label : item.result === "recognized" || item.result === "named" ? "未纳入主库" : "—",
      referenceUpdated: reference?.updated ?? "—",
      sourceBasis,
      note: item.note + " 来源：" + item.source + "；依据：" + sourceBasis + "；主库最后核对：" + (reference?.updated ?? "—") + "。",
    };
  });
}
...
export default function LedgerClient() {
  ...
  function exportComparison() {
    downloadCsv("法规版本比对结果.csv", comparisonRows.map((item) => ({
      ...
      原清单名称: item.originalTitle,
      原清单版本号: item.originalVersion ?? "",
      原清单发布日期: item.originalPublished ?? "",
      原清单实施日期: item.originalEffective ?? "",
      原清单来源: item.originalSource ?? "",
      原清单备注: item.originalNote ?? "",
      比对结果: compareMeta[item.result].label,
      主库参考标准号: item.referenceCode,
      主库参考名称: item.referenceTitle,
      主库参考状态: item.referenceStatus ?? "—",
      主库实施日期: item.effective,
      主库最后核对: item.referenceUpdated ?? "—",
      来源依据: item.sourceBasis ?? "—",
      官方来源机构: item.source,
      官方来源链接: item.href,
      处理说明: item.note,
    })));
  }
  function jumpToStatus(status: "active" | "upcoming") { setActiveTab(status); setCategory("all"); setPage(1); requestAnimationFrame(() => document.getElementById("changes")?.scrollIntoView({ behavior: "smooth", block: "start" })); }
  function exportPublicRegulations() { downloadRegulationsWorkbook("法规主库.xlsx", publicRegulations); }
  return <div className="mx-auto max-w-[1440px] px-6 py-8 lg:px-10 lg:py-10">
    <section className="mb-8"><div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><div className="mb-3 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.2em] text-[#d6534e]"><span className="size-2 rounded-full bg-[#e7655e]" /> Regulatory intelligence</div><h1 className="text-3xl font-semibold tracking-tight text-[#10213d] md:text-4xl">法规台账总览</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-500">把公开法规、国家标准和版本信息放在同一张台账里，先看状态，再核对版本变化。</p></div><Button variant="outline" onClick={exportPublicRegulations} className="shrink-0 border-[#bfd0e4] text-[#18345d] hover:bg-blue-50"><FileSpreadsheet className="mr-2 size-4" />导出公开法规 Excel</Button></div></section>
    <VersionCompare inputRef={inputRef} onFile={handleFile} rows={comparisonRows} summary={comparisonSummary} filter={comparisonFilter} onFilter={setComparisonFilter} uploadedName={uploadedName} uploadError={uploadError} onExport={exportComparison} onClear={() => { setComparisonRows([]); setComparisonFilter("all"); setUploadedName(""); setUploadError(""); }} />
    <section id="overview" className="mt-8 grid gap-4 md:grid-cols-2"><SummaryCard icon={<FileCheck2 className="size-5" />} label="现行法规" value={counts.active} caption="官方参考库中的现行版本" accent="blue" onClick={() => jumpToStatus("active")} /><SummaryCard icon={<CalendarClock className="size-5" />} label="即将实施" value={counts.upcoming} caption="已发布、尚未到实施日期的版本" accent="amber" onClick={() => jumpToStatus("upcoming")} /></section>
     <section id="changes" className="mt-8 rounded-2xl border border-slate-200 bg-white shadow-[0_8px_28px_rgba(16,33,61,0.04)]"><div className="border-b border-slate-100 p-5 md:p-6"><div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between"><div><h2 className="text-lg font-semibold text-[#10213d]">法规清单</h2><p className="mt-1 text-xs text-slate-400">公开法规参考库 · 仅展示现行与即将实施版本 · 最后核对 {LAST_CHECKED}</p></div><div className="relative w-full xl:max-w-sm"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" /><Input value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="搜索标准号、名称或来源" className="h-10 border-slate-200 pl-9" /></div></div><div className="mt-5 overflow-x-auto"><div className="flex min-w-max items-center gap-2"><span className="mr-1 text-xs font-medium text-slate-500">按类别筛选</span>{categories.map((item) => <button key={item} onClick={() => { setCategory(item); setPage(1); }} className={`rounded-full border px-3 py-1.5 text-xs transition ${category === item ? "border-[#18345d] bg-[#18345d] text-white" : "border-slate-200 bg-white text-slate-600 hover:border-slate-300 hover:bg-slate-50"}`}>{item === "all" ? `全部（${categoryCounts[item]}）` : `${item}（${categoryCounts[item]}）`}</button>)}</div></div><Tabs value={activeTab} onValueChange={(value) => { setActiveTab(value); setPage(1); }} className="mt-4"><TabsList className="h-auto flex-wrap justify-start gap-1 bg-slate-50 p-1"><TabsTrigger value="all">全部状态</TabsTrigger><TabsTrigger value="active">现行（{counts.active}）</TabsTrigger><TabsTrigger value="upcoming">即将实施（{counts.upcoming}）</TabsTrigger></TabsList></Tabs></div><div className="divide-y divide-slate-100">{pagedRegulations.map((item) => { const meta = statusMeta[item.status]; return <div key={item.id} role="button" tabIndex={0} onClick={(event) => { if ((event.target as HTMLElement).closest("input")) return; setSelected(item); }} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setSelected(item); } }} className="group grid w-full gap-4 p-5 text-left transition hover:bg-[#fbfcfe] md:grid-cols-[minmax(0,1fr)_160px_120px] md:items-center md:p-6"><div className="min-w-0"><div className="mb-2 flex flex-wrap items-center gap-2"><span className="font-mono text-xs font-semibold text-[#d6534e]">{item.code}</span><Badge variant="outline" className={`text-[11px] font-normal ${meta.className}`}><span className={`mr-1.5 inline-block size-1.5 rounded-full ${meta.dot}`} />{meta.label}</Badge><Badge variant="outline" className="border-slate-200 bg-slate-50 text-[11px] font-normal text-slate-500">{item.category}</Badge></div><h3 className="truncate text-[15px] font-semibold text-[#18345d] group-hover:text-[#d6534e]">{item.title}</h3><p className="mt-2 line-clamp-1 text-xs text-slate-500">{item.source} · 最后核对 {item.updated}</p></div><div className="text-xs text-slate-500"><span className="block text-[11px] text-slate-400">生效/关注日期</span><span className="mt-1 block font-medium text-slate-700">{item.effective}</span></div><div className="flex items-center justify-between text-xs font-medium text-[#d6534e] md:justify-end md:gap-2">查看详情 <ChevronRight className="size-4 transition group-hover:translate-x-0.5" /></div></div>; })}{!filtered.length && <div className="p-12 text-center text-sm text-slate-500">没有匹配的法规记录。</div>}</div><div className="flex flex-col gap-4 border-t border-slate-100 px-5 py-4 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between md:px-6"><div className="flex items-center gap-2"><span>每页显示</span><Select value={pageSize} onValueChange={(value) => { setPageSize(value); setPage(1); }}><SelectTrigger size="sm" className="h-8 w-[96px] border-slate-200 bg-white"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="10">10 条/页</SelectItem><SelectItem value="20">20 条/页</SelectItem><SelectItem value="50">50 条/页</SelectItem></SelectContent></Select><span>共 {filtered.length} 条 · 第 {currentPage}/{totalPages} 页</span></div><Pagination className="mx-0 w-auto justify-end"><PaginationContent><PaginationItem><PaginationPrevious href="#changes" onClick={(event) => { event.preventDefault(); if (currentPage > 1) setPage(currentPage - 1); }} className={currentPage === 1 ? "pointer-events-none opacity-40" : ""} /></PaginationItem><PaginationItem><PaginationLink href="#changes" isActive onClick={(event) => event.preventDefault()}>{currentPage}</PaginationLink></PaginationItem><PaginationItem><PaginationNext href="#changes" onClick={(event) => { event.preventDefault(); if (currentPage < totalPages) setPage(currentPage + 1); }} className={currentPage === totalPages ? "pointer-events-none opacity-40" : ""} /></PaginationItem></PaginationContent></Pagination></div></section>
    <section id="sources" className="mt-8 grid gap-4 lg:grid-cols-[1.3fr_0.7fr]"><div className="rounded-2xl border border-slate-200 bg-[#10213d] p-6 text-white md:p-7"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#f3a19c]">Official sources</p><h2 className="mt-2 text-xl font-semibold">官网入口与更新依据</h2></div><BellRing className="size-5 text-[#f3a19c]" /></div><div className="mt-6 grid gap-3 sm:grid-cols-2"><SourceLink title="国家药监局" href="https://www.nmpa.gov.cn/" /><SourceLink title="医疗器械技术审评中心" href="https://www.cmde.org.cn/" /><SourceLink title="国家标准全文公开平台" href="https://openstd.samr.gov.cn/" /><SourceLink title="国家药监局政务服务门户" href="https://zwfw.nmpa.gov.cn/" /></div></div></section>
...
function VersionCompare({ inputRef, onFile, rows, summary, filter, onFilter, uploadedName, uploadError, onExport, onClear }: { inputRef: RefObject<HTMLInputElement | null>; onFile: (event: ChangeEvent<HTMLInputElement>) => void; rows: ComparisonRow[]; summary: Record<CompareStatus, number>; filter: CompareStatus | "all"; onFilter: (filter: CompareStatus | "all") => void; uploadedName: string; uploadError: string; onExport: () => void; onClear: () => void }) {
  ...
  return <section id="compare" className="rounded-2xl border border-[#d8e2f0] bg-white shadow-[0_8px_28px_rgba(16,33,61,0.05)]">
    <div className="flex flex-col gap-5 border-b border-slate-100 p-5 md:flex-row md:items-start md:justify-between md:p-6">
      <div><div className="flex items-center gap-2"><div className="flex size-10 items-center justify-center rounded-xl bg-blue-50 text-blue-700"><FileSpreadsheet className="size-5" /></div><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-blue-600">Version check</p><h2 className="text-lg font-semibold text-[#10213d]">法规版本比对</h2></div></div><p className="mt-4 max-w-3xl text-sm leading-6 text-slate-500">先下载标准模板，填写法规名称；有编号时填写标准号，没有编号的文件也可以只填写名称。所有结果只与本站最新法规主库比对：历史版本会指向主库中的最新版本，无编号文件会按名称识别。</p></div>
      <div className="flex shrink-0 flex-wrap gap-2"><Button variant="outline" onClick={() => downloadCsv("法规版本比对模板.csv", [{ 标准号: "", 法规名称: "", 版本号: "", 发布日期: "", 实施日期: "", 来源: "", 备注: "" }])} className="border-slate-200"><Download className="size-4" />下载标准模板</Button><input ref={inputRef} type="file" accept=".xlsx,.csv,.tsv,.xls" onChange={onFile} className="hidden" /><Button onClick={() => inputRef.current?.click()} className="bg-[#18345d] text-white hover:bg-[#10213d]"><Upload className="mr-2 size-4" />上传填写后的模板</Button>{rows.length > 0 && <Button variant="outline" onClick={onExport} className="border-slate-200"><Download className="mr-2 size-4" />导出结果</Button>}</div>
    </div>
    <div className="px-5 pt-5 md:px-6">{uploadedName ? <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-blue-100 bg-blue-50/60 px-4 py-3 text-sm text-blue-900"><span><span className="font-medium">已载入：</span>{uploadedName}</span><button onClick={onClear} className="inline-flex items-center gap-1 text-xs text-blue-700 hover:text-blue-950"><X className="size-3.5" />清除</button></div> : <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-4 text-sm text-slate-500">模板中“法规名称”为必填列；有编号时填写“标准号”，没有编号的文件也可以只填写名称。版本号、发布日期、实施日期、来源、备注可按需填写。网站支持 UTF-8 和 GBK/GB18030 编码的中文 CSV 文件。</div>}{uploadError && <p className="mt-3 rounded-lg bg-red-50 px-4 py-3 text-sm text-red-700">{uploadError}</p>}</div>
    {rows.length > 0 && <><div className="border-b border-slate-100 px-5 pt-5 md:px-6"><p className="mb-3 text-xs text-slate-500">点击状态卡片可筛选下方比对明细；再次点击当前卡片可恢复全部结果。</p><div className="grid gap-3 pb-5 sm:grid-cols-3 lg:grid-cols-6"><CompareStat label="需要替换" value={summary.replace} tone="red" active={filter === "replace"} onClick={() => onFilter(filter === "replace" ? "all" : "replace")} /><CompareStat label="即将实施" value={summary.upcoming} tone="amber" active={filter === "upcoming"} onClick={() => onFilter(filter === "upcoming" ? "all" : "upcoming")} /><CompareStat label="现行" value={summary.active} tone="green" active={filter === "active"} onClick={() => onFilter(filter === "active" ? "all" : "active")} /><CompareStat label="按名称识别" value={summary.named} tone="blue" active={filter === "named"} onClick={() => onFilter(filter === "named" ? "all" : "named")} /><CompareStat label="已识别但未纳入主库" value={summary.recognized} tone="blue" active={filter === "recognized"} onClick={() => onFilter(filter === "recognized" ? "all" : "recognized")} /><CompareStat label="未匹配" value={summary.unmatched} tone="gray" active={filter === "unmatched"} onClick={() => onFilter(filter === "unmatched" ? "all" : "unmatched")} /></div></div>{visibleRows.length > 0 ? <div className="overflow-x-auto"><table className="min-w-[980px] w-full text-left text-xs"><thead className="bg-slate-50 text-slate-500"><tr><th className="px-5 py-3 font-medium md:px-6">文控清单（原版本）</th><th className="px-5 py-3 font-medium">比对结果</th><th className="px-5 py-3 font-medium">主库最新参考 / 官方入口</th><th className="px-5 py-3 font-medium">实施日期</th><th className="px-5 py-3 font-medium">说明</th></tr></thead><tbody className="divide-y divide-slate-100">{visibleRows.map((item, index) => { const meta = compareMeta[item.result]; return <tr key={`${item.originalCode}-${index}`} className="align-top"><td className="max-w-[250px] px-5 py-4 md:px-6"><p className="font-mono font-semibold text-[#d6534e]">{item.originalCode || (item.result === "named" ? "无编号文件" : "未识别文件")}</p><p className="mt-1 leading-5 text-slate-500">{item.originalTitle}</p></td><td className="px-5 py-4"><Badge variant="outline" className={meta.className}>{meta.label}</Badge></td><td className="max-w-[310px] px-5 py-4">{item.href ? <a href={item.href} target="_blank" rel="noreferrer" className="font-medium text-[#18345d] hover:text-[#d6534e]">{item.referenceCode}<span className="mt-1 block leading-5 text-slate-500">{item.referenceTitle}</span></a> : <span className="text-slate-400">{item.referenceCode}</span>}</td><td className="px-5 py-4 whitespace-nowrap text-slate-600">{item.effective}</td><td className="max-w-[340px] px-5 py-4 leading-5 text-slate-500">{item.note}</td></tr>; })}</tbody></table></div> : <div className="border-t border-slate-100 p-8 text-center text-sm text-slate-500">当前筛选没有比对结果。<button onClick={() => onFilter("all")} className="ml-2 font-medium text-[#18345d] underline underline-offset-2">显示全部结果</button></div>}</>}
  </section>;
}
